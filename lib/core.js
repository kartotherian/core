 ...
 *
 */
core.init = function init(app, rootDir, npmLoader, npmResolver) {
  return Promise.try(() => {
    _app = app;
    _rootDir = rootDir;
 ...
 */
core.registerTileliveModuleAsync = function registerTileliveModuleAsync(moduleName) {
  ...
  if (!core._registeredModules[moduleName]) {
    ...
    return Promise.resolve(_npmLoader(moduleName)).then((module) => {
